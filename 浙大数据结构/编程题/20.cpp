...
#define INF 65535
int dist[501];
int map[501][501]; /* 2 城市之间的距离~ */
int cost[501][501]; /* 2 城市之间的路费~ */
int MCost[501];
bool visited[501] = {false}; /* 判断该点是否已经求出最短路 */
/*  dijkstra 求最短路的变种~
*/
void dijkstra(int v0, int v, int d) {
    dist[v0] = 0; visited[v0] = true; /* 初始状态 v 节点属于集合 */
    int i, w;
    for (i = 1; i < v; i++) { /* 开始主循环 每次求得 v 到某个顶点的最短路径 并加 v 到集合 */
        int MIN = INF; /* 当前所知离 v0 最近的节点 */
        for (w = 0; w < v; w++) {
            if (!visited[w] ) { /* 节点在 结合 V-S 中 */
